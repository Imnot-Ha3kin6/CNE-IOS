...
class ApplicationMain
{
	...
	{
		lime.system.System.__registerEntryPoint("CodenameEngine", create);

		#if (js && html5)
		#if (munit || utest)
		lime.system.System.embed("CodenameEngine", null, 0, 0);
		#end
		#else
		create(null);
		#end
	...
	public static function create(config):Void
	{
		...
		app.meta["build"] = "6";
		app.meta["company"] = "Yoshman29";
		app.meta["file"] = "CodenameEngine";
		app.meta["name"] = "Friday Night Funkin' - Codename Engine";
		app.meta["packageName"] = "com.yoshman29.codenameengine";
		app.meta["version"] = "0.1.0-legacy";
	...
	macro public static function getEntryPoint()
	{
		...
		switch (Context.follow(Context.getType("funkin.backend.system.Main")))
		{
			case TInst(t, params):
				...
				if (hasMain)
				{
					return Context.parse("@:privateAccess funkin.backend.system.Main.main()", Context.currentPos());
				}
				...
				{
					Context.fatalError("Main class \"funkin.backend.system.Main\" has neither a static main nor a constructor.", Context.currentPos());
				}

			default:

				Context.fatalError("Main class \"funkin.backend.system.Main\" isn't a class.", Context.currentPos());
		}
...
@:keep @:dox(hide) class DocumentClass extends funkin.backend.system.Main {}
#else
class DocumentClass
{
